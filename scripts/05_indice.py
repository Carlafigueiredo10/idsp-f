...
def classificar(df: pd.DataFrame, corte: str) -> tuple[pd.DataFrame, dict]:
    ...
    a_nivel = df.A_pct.map(lambda v: "baixo" if v < a_lim else "alto")
    b_nivel = df.B_pct.map(lambda v: "alto" if v >= b_lim else "baixo")
    ...
    df["posicao"] = df.gravidade.rank(method="min", ascending=False).astype(int)
...
def main():
    ...
    for lente in LENTES:
        ...
        df[["n_ativos", "n_abono"]] = df[["n_ativos", "n_abono"]].fillna(0).astype(int)
        df["lente"] = lente
        df["A_raw"] = (df.n_ativos / df.populacao * 10_000).round(2)
        df["B_raw"] = (df.n_abono / df.n_ativos.replace(0, pd.NA)).astype(float).round(4)
        ...
        df.loc[df.n_ativos < smin, ["A_raw", "B_raw"]] = pd.NA
        saida[lente] = df.sort_values("posicao")

    uf_out = pd.concat(saida.values(), ignore_index=True)
    cols = ["uf", "nome", "regiao", "lente", "populacao", "n_ativos_pub", "n_abono_pub",
            "A_raw", "B_raw", "A_pct", "B_pct", "quadrante", "classe", "gravidade", "posicao", "flags"]
    csv_uf = uf_out[cols].rename(columns={"n_ativos_pub": "n_ativos", "n_abono_pub": "n_abono"})
    csv_uf.to_csv(PROCESSED / "idspf_uf.csv", index=False, encoding="utf-8")

    # por grupo do núcleo
    g1 = f1[f1.grupo_nucleo != ""].groupby(["uf", "grupo_nucleo"], as_index=False).n_ativos.sum()
    g2 = (f2[f2.grupo_nucleo != ""].groupby(["uf_residencia", "grupo_nucleo"], as_index=False).n_abono.sum()
    ...
    gr["B_raw"] = (gr.n_abono / gr.n_ativos.replace(0, pd.NA)).astype(float).round(4)
