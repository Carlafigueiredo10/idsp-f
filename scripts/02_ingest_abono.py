...
def main():
    ...
    header = list(pd.read_csv(path, sep=sep, encoding=enc, nrows=0).columns)
    mapa = mapear_cabecalho(header)
    log(f"F2: {path.name} sep={sep!r} enc={enc} mapa={mapa}")
    ...
    write_json(INTERIM / "meta_f2.json", dict(
        ...
        mapa_colunas=mapa, linhas_lidas=tot, linhas_mantidas=mantidos, abonos=n_total,
